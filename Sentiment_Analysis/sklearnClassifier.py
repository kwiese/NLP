#Author: Kyle Wiese
#Programming Assignment 4 (Sklearn Implementation)
...
def main():
	...
	pos_rev_words = []
	for i in range(len(pos_documents)):
		for j in range(len(pos_documents[i])):
			pos_rev_words.append(pos_documents[i][j])

	#get words that are only used in the negative reviews
	neg_words = list(set(neg_rev_words) - set(pos_rev_words))

	#find all words that arent common or punctuation
	stop_words = sw.words("english")
	all_words = FreqDist(w.lower() for w in neg_words 
				if w.lower() not in string.punctuation 
				and w.lower() not in stop_words)

	#grab first 3000 of the remaining words
	neg_features = all_words.keys()[:3000]
	...
	for i in xrange(1,11):
		...
		classifier = SklearnClassifier(SVC(C=8, gamma=10), sparse=False).train(train_set)
